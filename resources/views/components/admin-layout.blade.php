...
<body class="h-full bg-gray-50 text-gray-800">
    ...
    <div class="min-h-full">
        ...
        <!-- New admin navbar (kept color palette, different design) -->
        <x-admin-navbar />

        {{-- Render sidebar once for all admin pages. Pages can set a section 'sidebar_active' to control active menu --}}
        @php
            $sidebarActive = trim($__env->yieldContent('sidebar_active')) ?: '';
            $hasSidebar = $sidebarActive !== '';
        @endphp

        {{-- Render sidebar based on user role when a page requests it via section('sidebar_active') --}}
        @if ($hasSidebar)
            @php
                $role = Auth::user()->role;
            @endphp
            @if ($role === 'admin')
                <x-admin-sidebar :active="$sidebarActive" />
            @elseif ($role === 'juri')
                <x-juri-sidebar :active="$sidebarActive" />
